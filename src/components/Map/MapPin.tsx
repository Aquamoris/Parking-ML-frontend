...
const MapPin = ({ fill, address, amount }: { fill: string, address?: string, amount? : number }) => {
    ...
    return (
        <>
            ...
            {modal ? <Modal address={address} close={() => setModal(false)} color={fill} amount={amount}/> : null}
        </>
    );
};
